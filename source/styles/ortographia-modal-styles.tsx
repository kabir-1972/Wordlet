...
const OptionsTabs=(props: OptionsTabProps)=>{
    ...
    <View style={[{flexGrow: 1}, props.selected?styles.chosenDifficulty:null]}>
        ...
        <View style={[styles.difficultyTabContent]}>
            <TabText style={{lineHeight: 21, textAlign: 'center'}}>{props.mode}</TabText>
        </View>
                </ImageBackground>     
        </Pressable>
    </View>
...
export const OrtographiaChoice={
  type : 1,
}
...
export const CompleteOrtographiaModal=(props:wordleStyleProps)=>{
  const [selected, setSelected] = useState(1);
  
  const handlePress = (key: number) => {
    setSelected(key);
        OrtographiaChoice.type = key;
   }
    return(
        <View>
            <ModalText style={styles.modalText}>Select Difficulty:</ModalText>
            <View style={styles.difficultyTabs}>
                <OptionsTabs mode = "Mode 1" difficultyChooseBtn = {()=>handlePress(1)} selected={selected===1} difficultyTabImage={props.difficultyTabImage}></OptionsTabs>
                <OptionsTabs mode = "Mode 2" difficultyChooseBtn = {()=>handlePress(2)} selected={selected===2} difficultyTabImage={props.difficultyTabImage}></OptionsTabs>
            </View>
            <Text style={{fontFamily: 'Wordlet-Regular', fontSize: 16, textAlign: 'center', color: '#ebfcf7', lineHeight: 20}}>
                {selected==1?"From four options, pick the one that is the correct spelling":"Each options will have a pair of words, pick the one that has one correct spelling with the incorrect one"}
            </Text>
        </View>
    );
}
