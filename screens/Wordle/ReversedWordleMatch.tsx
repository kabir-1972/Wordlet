...
import {saveWordleDataToFile,
  readWordleDataFile,
  updateAccessoryDataInPreviousWordleFile,
...
import { HeaderInMatch } from './Wordle-Header-inmatch';
...
type WordleMatchRouteProp = RouteProp<RootStackParamList, 'ReversedWordleMatch'>;
export type NavigationProp =NativeStackNavigationProp<RootStackParamList, 'Home'>;
  ...
  const [misPlacedLetters, setMisPlacedLetters]=React.useState<string[]>([]);
  ...
  const _gameName='Reversed-Wordle';
...
[untouchedLetters,fileRead]
  ...
  */
  ...
  const searchForWordInFiles=async(word_:string,)=>{
    let userInputWord=word_;
    let word=word_;
    word=word_.split('').reverse().join('');
  ...
  try {
    ...
    if (!wordlist.includes(word)) {
      ...
      if(targetWord!=undefined){
        for(let i=0;i<word.length;i++){
          if(targetWord[i]==userInputWord[i]){
            correctlyPlacedLetterSet.push(userInputWord[i]);
            if (untouchedLettersTemplate.indexOf(userInputWord[i]) !== -1) 
            untouchedLettersTemplate.splice(untouchedLettersTemplate.indexOf(userInputWord[i]), 1);
          }
          else if(targetWord.includes(userInputWord[i])&&!correctlyPlacedLetterSet.includes(userInputWord[i])){
            misPlacedLetterSet.push(userInputWord[i]);
            if (untouchedLettersTemplate.indexOf(userInputWord[i]) !== -1) 
            untouchedLettersTemplate.splice(untouchedLettersTemplate.indexOf(userInputWord[i]), 1);
          }
          else if(!correctlyPlacedLetterSet.includes(userInputWord[i])&&!misPlacedLetterSet.includes(userInputWord[i])){
            notToBePlacedLetterSet.push(userInputWord[i]);
            if (untouchedLettersTemplate.indexOf(userInputWord[i]) !== -1) 
            untouchedLettersTemplate.splice(untouchedLettersTemplate.indexOf(userInputWord[i]), 1);
          }
      }
    const correctlyPlacedLetterSetTemplate=[... new Set(correctlyPlacedLetterSet)];
    const misPlacedLetterSetTemplate=[... new Set(misPlacedLetterSet)];
    ...
    setMisPlacedLetters(misPlacedLetterSetTemplate);
...
/*    The buttons and their animations [LightWand and Starthrow]    */
  ...
  setMisPlacedLetters(savedData.misPlacedLetters);
  ...
  async function fetchWord() {
      ...
      for (let i = 97; i <= 122; i++) {
        ...
        if(!word?.includes(theChar)
          ...
          &&!misPlacedLetters.includes(theChar)
...
/*
useEffect(() => {
  if(!scoreDataRead) return;
  async function fetchWord() {
      let word = await pickARandomWord(wordLength);
      console.log(word);
      if (!word) return;
      ...
      for (let i = 97; i <= 122; i++) {
        ...
        localUntouchedLetters=shuffleArray(localUntouchedLetters);
        setUntouchedLetters(localUntouchedLetters);
        await updateUntouchedLettersInPreviousWordleFile(savedDataFileName,localUntouchedLetters);
    }
    if(!previouslySavedData) fetchWord();

}, [previouslySavedData, scoreDataRead]);
*/

  if(!fileRead||!targetWord||targetWord.length==0) {
   return(
   <ImageBackground
...
type WordleRowProps={
 ...
 const WordleRow=(props:WordleRowProps)=>{
  ...
  function computeColors() {
    ...
    const result: ("green" | "yellow" | "gray")[] = Array(word.length).fill("gray");
...
animations.reduce((prev, anim) => {
  ...
  const getBoxColor = (index: number) => {
    ...
    if (colorArray[index] === "yellow") return "#ffcf56";
