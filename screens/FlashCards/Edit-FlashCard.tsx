...
type EditFlashCardRouteProp = RouteProp<RootStackParamList, 'EditFlashCard'>;

export type NavigationProp =NativeStackNavigationProp<RootStackParamList, 'Home'>;

  const App = () => {
  //const route=useRoute<EditFlashCardRouteProp>();
  const route={
    params:{
        name: "My Pack"
    }
  ...
  const [flashCardDataRead, setFlashCardDataRead]=useState(false);
...
useEffect(() => {
  ...
  const flashCardData=await RNFS.readFile(`${RNFS.DocumentDirectoryPath}/player-flash-cards/${route.params.name}.json`);
  const JSONFlashCardData=JSON.parse(flashCardData);
  
  setdescriptionOfPacket(JSONFlashCardData.description);
  setNameOfFlashCardPacket(route.params.name);
  const data=JSON.parse(JSONFlashCardData.data);
  
  let maxKey=0;
for (const key of Object.keys(data)) {
  const bothFaceData = JSON.parse(data[key]);
  await AsyncStorage.setItem("editCard" + key, JSON.stringify(bothFaceData));
  maxKey++;
}

  
  setNumberOfCards(maxKey);
  fetchCard(1);
...
const addCard = async(cardNumber: number)=>{
   ...
   await AsyncStorage.setItem("editCard"+cardNumber, JSON.stringify(cardData));
...
const fetchCard=async(cardNumber: number)=>{
  ...
  try{
    const cardData= await AsyncStorage.getItem("editCard"+cardNumber);
    if(cardData !==null){
...
const createCardPackAsFile=async()=>{
  ...
  for(let i = 0;i<arrayOfCardsToChoose.length; i++){
    const cardData=await AsyncStorage.getItem("editCard"+arrayOfCardsToChoose[i]);
    if(cardData) 
